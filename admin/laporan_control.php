...
if(isset($_GET['action']) && $_GET['action'] == "hapus") {
    ...
    if(mysqli_num_rows($query_warga)){
        ...
        // hapus verifikasi
        $sql_hapus_verifikasi = mysqli_query($koneksi," DELETE FROM verifikasi where warga_id = '$id_warga'");

        // hapus foto pendaftar
        if(file_exists('../uploads/'.$data_warga['foto'])){
            unlink('../uploads/'. $data_warga['foto']);
        }
        $sql_hapus_warga = mysqli_query($koneksi," DELETE FROM warga where id = '$id_warga'");

        // hapus user
        $sql_hapus_user = mysqli_query($koneksi," DELETE FROM users where id = '$id_user'");

        if(!$sql_hapus_verifikasi || !$sql_hapus_warga || !$sql_hapus_user) {
            die('Query Error: '. mysqli_error($koneksi));
        }
